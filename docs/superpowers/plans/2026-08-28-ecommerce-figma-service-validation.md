...
> **For agentic workers:** REQUIRED SUB-SKILLS: `figma-design-to-code`, `chrome:control-chrome`, `sites-building`, `sites-hosting`, and conditionally `computer-use` for Figma Desktop. If a social bitmap is missing, dispatch exactly one image-only subagent with ImageGen capability; the main agent never invokes ImageGen directly and remains sole Site editor/owner. Run only after the R18 binding plan is READY and Tasks1–16 are reviewed.
...
Top-level `validationPhase` discriminates `pre-extraction|final`. Both phases require sourceCommit, candidate/evidence/marker `{path,fileDigest64,contentHash:WireSha256}`, artifact-manifest/SHA256SUMS and final MCP/CLI/plugin RawDigest64, buildId, `daemonLaunch:'packed-dist-node'`, plus `daemonRun {pid,startedAt,daemonGenerationHash,buildId,packedMcpDigest}`. Pre-extraction egress contains only before/configured hashes/classes/TTL and configure audit IDs/hashes and requires empty trace arrays. Final egress additionally requires final unknown/resetConfigHash and reset audit IDs/hashes plus complete trace. No phase contains consentId, nonce, or credential.

`serviceOperations` is a strict `oneOf` discriminated by `evidenceType`, with common unique operationId, exact operationKind/name, `status:'succeeded'`, `resultHash:WireSha256`, completedAt, receiptHash, finalizerHash, daemonGenerationHash, and packedClientDigest. Every receipt generation equals `daemonRun.daemonGenerationHash`, completedAt is not before daemonRun.startedAt, and receipt kind/name/result/finalizer exactly matches current operation status and receipt route:
...
After V7 reset, run the same full flag set with `--phase final`. Final repeats immutable checks, validates the AdminAudit order `pending -> config-cas -> committed` for configure and reset, validates every operation receipt, parses canonical read artifacts, checks snapshot/export references, requires current unknown-fail-closed/resetConfigHash, and requires unresolved empty.
...
- [ ] Before loading any control credential, probe `127.0.0.1:3055` publicly. If a listener exists, use `LocalDaemonBindingVerifier`: `readLeaderLock` is only an untrusted coordination hint, while strict ping plus versioned follower challenge proof under the owner-secure generation credential authenticates PID/port/build/generation. A foreign/mismatched listener is a blocker and receives zero control credential; never kill it. A verified same-owner pre-existing daemon must be cleanly stopped by its owner, awaited, and port-close confirmed before continuing. Do not adopt it: this plan must retain the child it launches.
...
- [ ] Immediately after spawn, enter one outer V3-V7 `try/finally` state contract holding `{sitesDevChild,daemonChild,daemonRun,pluginSession,packedCli,egressChanged:false,cleanupErrors:[]}`. Every exit resets changed egress, closes clients/plugin, awaits the exact daemon and port close, and stops the Sites dev server. All cleanup errors remain blockers. If V1/V2 fails earlier, its local finally stops the Sites dev child.
...
- [ ] Save redacted `sfp egress status --json`, require prior unknown, configure the exact four classes for2h, then atomically set guard `egressChanged:true`. Query `sfp egress audit --since <runStartedAt> --json` and require pending, config-cas, committed records matching config metadata; record no consent. If TTL cannot cover validation, execute cleanup and restart.
...
- [ ] Invoke semantic tools only through the same-run packed generic CLI, for example `sfp tools call search_nodes --args-file .sfp/args/search.json --workspace <siteWorkspaceId> --target active --out .sfp/evidence/search.json` and `sfp tools call get_design_context --args-file .sfp/args/context.json --workspace <siteWorkspaceId> --target active --out .sfp/evidence/context.json`. Discover public `node-id=0-1` and record the canonical landing frame only from these strict canonical result artifacts, never Chrome layer inspection. Record packed CLI digest, operation ID, and `sfp operations evidence <id> --json` receipt for every call.
- [ ] Capture full context/SnapshotV1 for that frame through the generic CLI/service route and recursively follow nested section plans. Every section-referenced read must use `--out` and a nonnull read-result receipt artifact; depth/cycle/count failures require smaller service recaptures.
...
- [ ] Use exact generic calls for `get_screenshot`, `save_screenshots`, `save_image_fills`, `component_map`, `token_map`, `icon_map`, `export_tokens`, and any PDF/image export, each with canonical args, the same workspace/daemon, and `--out <workspace-relative>`. Immediately verify the operation evidence receipt kind/name/result/finalizer/generation and current artifact bytes/RawDigest64; no individual wrapper or unrelated old operation may substitute.
- [ ] Run `npm install -D ajv@8.17.1`, create schema/validator/negative tests and a `validationPhase:'pre-extraction'` skeleton with empty trace arrays, then run exact `node scripts/validate-service-evidence.mjs --phase pre-extraction --service-root ../../service --packed-runtime .sfp/validation-runtime --asset-root public --evidence service-evidence.json --schema service-evidence.schema.json`. Do not run final phase here.
...
- [ ] Reset product egress through the same packed CLI, require fresh unknown/resetConfigHash, query and verify reset audit records, add their redacted IDs/hashes, and change evidence to `validationPhase:'final'`. Any failure blocks deployment.
...
- [ ] Run final build while the retained dev server stays alive. Scan exact Site source plus generated `dist` for credentials, consent IDs, private URLs, absolute paths, raw snapshots/journals, and identifiers; fix and rerun. Commit final Site source/evidence and require Git index, committed source bytes, and packaging input bytes identical.
- [ ] Reload the current `sites-building`/`sites-hosting` skill instructions and invoke their exact `package-site.sh` preflight/package workflow. Require it returns an accessible exact hosting archive path; scan that archive byte-for-byte with the same forbidden-data rules and verify its source manifest hashes equal the committed Site Git bytes. If the archive is inaccessible or the skill does not expose it, stop and report a blocker; never claim it was scanned.
- [ ] Deploy privately from the exact scanned archive/Site-root Git repository. Public/shared access requires prior license clearance and explicit user approval.
