...
> **For agentic workers:** REQUIRED SUB-SKILLS: `figma-design-to-code`, `chrome:control-chrome`, `sites-building`, `sites-hosting`, conditionally `computer-use`; exactly one image-only subagent only if needed. Run after R25 READY and Tasks1-16 source-complete; main remains sole Site editor.
...
- Tasks1-16 are implemented/reviewed at the source-complete commit and R25 is READY. Only after V1 clean commit run one source-complete command; never mix runs.
...
`serviceOperations` contains only finalized successful extraction operations. Its strict rows bind unique operationId, kind/name, args/workspace/target/capture fingerprint, succeeded result and native evidence, generation/completedAt and packedClientDigest. Each row embeds the exact packed-CLI `OperationEvidenceViewV1`; the validator requires `serverVerified:true`, reciprocal succeeded status/receipt/finalizer projection, same daemon generation and current packed client. It never joins journal files itself.
...
The schema has no live negative-operation array, and V3 performs no reject/expiry/duplicate/conflict probe against the real Draft. Instead strict `sourceCompleteNegativeProofs` binds the same-run trusted `SourceCompleteEvidenceV1` file RawDigest64 plus exact pass/resultHash rows: `fake.workspace-policy` and `fake.network-policy` prove pre-egress policy rejection; `fake.workspace-policy` also binds its nonce reuse/runtime0 subclaim; `fake.approval-undo` proves reject/expiry/duplicate approval CAS; `fake.idempotency-journal` proves conflict/settled replay/outcome-unknown no-rerun. The validator rereads and schema/content-hash validates SourceCompleteEvidence, requires those unique pass rows and expected subclaim hashes, and rejects forged evidence-field copies.
...
`validate-service-evidence.test.mjs` must fail independently for RawDigest64/WireSha256 prefix swaps, one-nibble file tamper, wrong daemon generation/start time/build identity/packed digest, forged/duplicate/missing or old unrelated operation view, wrong target/capture intent, wrong receipt/finalizer projection, canonical read parse/hash/node mismatch, null/incomplete snapshot, missing/traversing/symlinked native manifest/member, count/order/byte/digest/source-ref mismatch, dangling section/asset reference, broken/reordered/foreign AdminAudit store response, missing/failed/forged trusted SourceComplete negative-proof row or file hash, candidate/evidence tuple mismatch, configured/final egress hash or mode mismatch, nonempty unresolved output, and pre/final phase-field crossover. There is no blanket hash rule or mocked-success CLI seam in final validation.
...
- [ ] Run the current Sites skill's exact root-level helper `scripts/package-site.sh <project> <archive>` once against that HEAD. The archive contains only `dist/**`. Before extraction list every tar member and reject absolute paths, `..`, backslash, symlink, hardlink, device, duplicate or non-`dist/` roots. Extract to a secure ignored temp and byte-compare all archive members with current `dist/`, including required `dist/.openai/hosting.json`. If project `drizzle/` exists, require its generated files map byte-for-byte to `dist/.openai/drizzle/**`; otherwise that archive subtree is absent. No `.openai/migrations` path is expected or allowed. Write ignored `.sfp/site-build-provenance.v1.json` outside package input, binding HEAD, product-manifest hash, dist/hosting/optional-drizzle/archive/member hashes; validate it separately.
...
- [ ] The final push used exact validated HEAD; archive contains only dist and byte-matches hosting plus optional drizzle mapping; one version was saved, access was resolved/re-resolved when needed, the correct private-or-approved deployment route was used, terminal deployment was polled, and the same retained Site tab opened the deployed URL before the Sites child stopped.
