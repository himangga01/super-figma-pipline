...
> **For agentic workers:** REQUIRED SUB-SKILLS: `figma-design-to-code`, `chrome:control-chrome`, `sites-building`, `sites-hosting`, conditionally `computer-use`; exactly one image-only subagent only if needed. Run after R27 READY and Tasks1-16 source-complete; main remains sole Site editor.
...
- Tasks1-16 are implemented/reviewed at the source-complete commit and R27 is READY. Only after V1 clean commit run one source-complete command; never mix runs.
