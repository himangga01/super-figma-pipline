...
> **For agentic workers:** REQUIRED SUB-SKILLS: `figma-design-to-code`, `chrome:control-chrome`, `sites-building`, `sites-hosting`, and conditionally `computer-use`. If social art is missing, dispatch exactly one image-only subagent; main never invokes ImageGen and remains sole editor. Run only after R19 READY.
...
- [ ] Before probing or spawning, enter the outer V3-V7 guard with retained Sites child and null daemon/client/plugin plus pessimistic cleanup state. Every exit attempts reset if flagged, closes clients, awaits daemon/port, and stops Sites; cleanup errors remain blockers.
...
- [ ] Set guard `egressChanged:true` before configure, then configure four classes for2h. Query egress audit and require one transaction's pending, cas-intent, committed records with exact expected/desired config hashes; record no consent. Failure triggers outer cleanup.
...
- [ ] Reload current Sites skills and call `create_site` before final build. Persist project_id; retain returned source write credential only in memory/owner-private temp, never files/logs.
...
- [ ] Call the current Sites save-version/deploy flow with project_id, scanned tar.gz and exact commit_sha. Reverify owner-only/private access, poll until terminal success, open the private URL, then let the outer guard stop/await the Sites dev server. Public access requires separate approval.
