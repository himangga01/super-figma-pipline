...
describe('closed staged change manifest', () => {
  it.each(['7A', 'review-2026-09-05'])(
    ...
    async slice => {
      ...
      await writeFile(
        join(root, `service/capabilities/task-${slug}-authority-classes.json`),
        `${JSON.stringify(
          {
            schemaVersion: 1,
            slice,
            allowedPaths: [
              `service/capabilities/change-manifests/task-${slug}.json`,
              'service/example.ts',
      ...
      const verified = spawnSync(process.execPath, [script, '--slice', slice], {
        cwd: root,
        encoding: 'utf8',
        env,
      });
      ...
      const dirty = spawnSync(process.execPath, [script, '--slice', slice], {
        cwd: root,
        encoding: 'utf8',
        env,
      });
