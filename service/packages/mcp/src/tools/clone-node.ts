import { z } from 'zod';

import type { RawToolSpec } from './spec.js';

export const CLONE_NODE_TOOL_NAME = 'clone_node';

export const cloneNodeTool: RawToolSpec = {
  name: CLONE_NODE_TOOL_NAME,
  description:
    'Duplicate a node, including its full subtree, and append the copy as the last child of ' +
    'the same parent. Cloning a component instance keeps it an instance; to make ' +
    'a fresh instance of a component use create_instance instead. Returns ' +
    '{ ok, nodeId, name, type } for the copy.',
  inputSchema: z.object({
    nodeId: z.string().describe('Node id to clone'),
  }),
  kind: 'write',
};
