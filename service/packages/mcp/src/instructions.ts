 ...
 */
export const SERVER_INSTRUCTIONS = `Figwright connects this codebase to a Figma file in both directions. It needs the Figwright plugin open in Figma; \`ping\` reports whether it is, and design tools cannot work until it is.

Reading a design into code — ground it, never guess it. \`get_design_context\` is the source of truth for every measurement, colour, font and spacing value. Do not read those off an image: \`get_screenshot\` exists to verify what you built, not to measure what to build. Build structure from the tree, not from coordinates: a frame is a container, its nesting is your markup's nesting, and each frame's layout object gives the exact flex/grid mode, padding, gap and alignment to emit — deriving spacing from child x/y as per-child margins or absolute offsets is the same error as eyeballing a colour, and it is what makes generated layouts break on resize. Before generating anything, join the design to what this project already has — \`component_map\`, \`token_map\` and \`icon_map\` match Figma components, variables and icons against the local codebase, and a match means reference the existing one rather than write an equivalent. Real pixels (logos, photography) come from \`save_screenshots\` or \`save_image_fills\`; never approximate them in markup.
